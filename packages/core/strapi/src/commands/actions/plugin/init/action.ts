...
interface ActionOptions extends CLIContext, Pick<InitOptions, 'silent' | 'debug'> {}

export default async (path: string, { logger, cwd, silent, debug }: ActionOptions) => {
  try {
    /**
...
const PLUGIN_TEMPLATE = defineTemplate(async ({ logger, gitConfig }) => {
  let repo: {
    source?: string;
    owner?: string;
    name?: string;
  ...
  return {
    ...
    async getFiles(answers) {
          ...
          switch (name) {
            ...
            case 'server-code': {
              ...
              files.push({
                ...
                contents: outdent`
                    ...
                    module.exports = require('./dist/server');3
                `,
              });
            ...
            case 'typescript': {
              ...
              if (isTypescript) {
                ...
                devDepsToInstall.push('@strapi/typescript-utils');
              ...
              if (isRecord(pkgJson.exports['./strapi-server'])) {
                if (isTypescript) {
                  ...
                  files.push(...serverTypescriptFiles);
                } else {
                  const { serverJavascriptFiles } = await import('./files/server');

                  files.push(...serverJavascriptFiles);
                }
              }
      ...
      try {
        ...
        pkgJson.dependencies = await resolveLatestVerisonOfDeps([
          ...Object.keys(pkgJson.dependencies),
        ]);
        pkgJson.peerDependencies = await resolveLatestVerisonOfDeps([
